...
export class GlobalBackupManager {
  ...
  private readonly BACKUP_INTERVAL_MINUTES = 30; // 30 minutes
  ...
  async startGlobalBackupScheduler(): Promise<void> {
    ...
    console.log('🚀 Starting backup scheduler - checks every 30 minutes');
    ...
    // Run backup check immediately on startup
  ...
  private async checkAndBackup(): Promise<void> {
    try {
      ...
      // If no previous backup or 30+ minutes have passed, create backup
      if (!lastBackupTime || (now - lastBackupTime) >= (this.BACKUP_INTERVAL_MINUTES * 60 * 1000)) {
  ...
  private async getLastBackupTime(): Promise<number | null> {
    try {
      ...
      return new Date(latestBackup[0].createdAt).getTime();
    } catch (error) {
  ...
  async getBackupStatus() {
    ...
    if (lastBackupTime) {
      nextBackupTime = lastBackupTime + (this.BACKUP_INTERVAL_MINUTES * 60 * 1000);
      
      if (nextBackupTime > now) {
        minutesUntilNext = Math.ceil((nextBackupTime - now) / (60 * 1000));
      } else {
        // Backup is overdue
        nextBackupTime = now;
        minutesUntilNext = 0;
    ...
    const lastBackupFormatted = lastBackupTime 
      ? new Date(lastBackupTime).toLocaleString('en-US', {
          year: 'numeric',
          month: 'numeric',
          day: 'numeric',
          hour: 'numeric',
